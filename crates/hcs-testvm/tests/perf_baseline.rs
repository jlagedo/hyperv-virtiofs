...
//! HVFS_PB_REPEATS (3), HVFS_PB_ATTEMPTS (4).
#![cfg(windows)]
...
fn perf_baseline_over_virtiofs() {
    ...
    eprintln!("params: seqmb={seqmb} meta={meta} rand={rand} repeats={repeats} initrd={initrd}");
    ...
    for run in 1..=repeats {
        ...
        for attempt in 1..=attempts {
            ...
            if let Some(samples) = try_perfbench(&kernel, &initrd, &ws, seqmb, meta, rand) {
                got = Some(samples);
                break;
            }
        }
    ...
    md.push_str(&format!(
        "- params: `seqmb={seqmb}` `meta={meta}` `rand={rand}` · runs ok: **{runs_ok}/{repeats}** · attempts/run: {attempts}\n",
    ));
    md.push_str(&format!(
        "- kernel: `{}`\n\n",
    ...
    let json = serde_json::json!({
        "params": { "seqmb": seqmb, "meta": meta, "rand": rand, "repeats": repeats, "runs_ok": runs_ok },
